...
define("IGK_ARMONIC_VERSION", "2.1");
...
function igk_treat_show_version(){
		$const = "constant";
	igk_wln(<<<EOF
This is igkdev trait php source code CLI
author: C.A.D. BONDJE DOUE
copyright: IGKDEV @ 2019
version : {$const('IGK_ARMONIC_VERSION')}
EOF
);

}
function igk_treat_show_usage(){
	igk_treat_show_version();
	$const = "constant";
	igk_wln(<<<EOF
...
igk_treat_reg_command("-d, --inputDir", function($v, $command, $c){
	if ($command->waitForNextEntryFlag){
		...
		$ifolder = null;
		$ignore_dir = igk_treat_get_ignore_regex($command);	
		$v_fctreat = function($file){
			return preg_match("/\.(pinc|ph(p|tml))$/", $file);
		};
		if (isset($command->ignoreDirs) && $command->ignoreDirs){
			$v_fctreat = function($file) use ($command){
		...
		foreach(igk_io_getfiles($dir, "/(.)+/", true, $ignore_dir)  as $file){
			...
			if ($v_fctreat($file)){ // preg_match("/\.(pinc|ph(p|tml))$/", $file)){
