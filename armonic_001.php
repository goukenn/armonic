...
function igk_treat_source_expression($options=null){
	...
	if ($defExpression==null){
		...
		$q_txt = igk_treat_get($m->options).substr($t, 0,$start);		 
		$si= substr($q_txt, $q->markOffset );
		// igk_wln("item::: ". trim($si));
		$q->markOffset += strlen($si)+1;// $next;
		$q->items[] = trim($si);
	};
	...
	function start_array($m, $t, $start, $bracket=1){
		...
		$m->options->arrayEntity[] = (object)[
			"start" => strlen($v_o)+$start,
			"markOffset"=>strlen($v_o)+$start+1,
	...
	array_unshift($tab, (object)array(
		...
		"mode"=>"*",
		"pattern"=>"/(^|\\s*|[^a-zA-Z0-9])(?P<operator>(for|if|elseif|while|do|switch|foreach|try|finaly|catch|array))\\s*(\(|\{|$)/",
		"callback"=>function(& $t, $start, & $offset, $m){
			...
			if ($m->options->FormatText){	
				...
				switch($g){
					...
					case ",":
						...
						if (isset($m->options->arrayDepth) && ($m->options->arrayDepth>0)){
							$q = $m->options->arrayEntity[count($m->options->arrayEntity)-1];
							update_array_item($q, $v.$g, strlen($v), $m);
						}
					break; 
					case "as":
						$g = " ".$g." ";
