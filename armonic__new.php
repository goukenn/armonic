	...
	array_unshift($tab, (object)array(
		...
		"callback"=>function(& $t, $start, & $offset, $m){
			...
			if ($d=="}"){
				...
				// igk_wln("end bracket detected: ".igk_env_count(__FUNCTION__) ,
				// "fM: ".$m->options->FormatText,
				// "INDENT: ".$_indent,
				// "BracketVar: ".$m->options->bracketVarFlag);
				if ($m->options->FormatText){
				...
				default:
					...
					if ( ($d==")") && ($m->options->conditionDepth<=0)){
						if(igk_getv($m->options, "multiLineFlag")==1){
							$s = $m->options->LF.$s;
							igk_treat_reset_flags($m->options); 
						}				
